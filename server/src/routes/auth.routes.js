...
router.post('/login', async (req, res) => {
    ...
    const token = jwt.sign(
        { id: resul.rows[0].id, email: resul.rows[0].email, username: resul.rows[0].username, money: resul.rows[0].money },
        jwtSecret,
        { expiresIn: '1h' } 
    );
...
router.post('/register', async (req, res) =>{
    ...
    if (username.length < 3) {
        return res.json({ success: false, message: 'Nazwa musi mieć co najmniej 3 znaki' });
    }
