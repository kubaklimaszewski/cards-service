...
document.getElementById('reg-username').addEventListener('change', (event) => {
    ...
    if (!usernamePattern.test(username)) {
        usernameError.textContent = "Wprowadź poprawną nazwę (tylko litery i cyfry)";
...
document.getElementById('reg-email').addEventListener('change', (event) => {
    ...
    const emailPattern = /^[a-zA-Z][a-zA-Z0-9]*(?:\.[a-zA-Z0-9]+)*@[a-zA-Z0-9]{2,}\.[a-zA-Z]+$/;
...
document.getElementById('reg-password').addEventListener('change', (event) => {
    ...
    if (!passwordPattern.test(password)) {
        passwordError.textContent = "Wprowadź poprawne hasło (co najmniej 8 znaków, po jednej dużej i małej literze oraz cyfrze)";
        submit.disabled = true;
    } else {
...
document.getElementById('reg-password-confirm').addEventListener('change', (event) => {
    const password = event.target.value ;
